...
final class CommandPackageResultWriter {
    ...
    List<OutputLine> lines(PackageResult result, String suffix) {
        List<OutputLine> lines = new ArrayList<>();
        lines.add(OutputLine.success(CommandPackageSupport.packageSummary(result) + suffix));
    ...
    record OutputLine(OutputLineKind kind, String message) {
