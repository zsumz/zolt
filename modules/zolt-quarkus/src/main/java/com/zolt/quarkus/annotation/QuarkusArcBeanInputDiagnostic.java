...
final class QuarkusArcBeanInputDiagnostic {
    private QuarkusArcBeanInputDiagnostic() {
    }

    static List<?> consumeMulti(BuildContext context, Class<?> buildItemClass) {
        try {
    ...
    static String formatTestClassBeanItems(List<?> items, List<String> testClasses) {
    ...
    static String formatAdditionalBeanItems(List<?> items, List<String> testClasses) {
        if (testClasses.isEmpty()) {
            return "count=" + items.size() + ",selected=<none>";
        }
        return "count="
