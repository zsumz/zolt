        ...
        if (!buildChainBuilderClassloaderSplit(result.output())) {
            ...
            if (profileTestClassBeanMissing(result.output())) {
                return "error: Quarkus annotation test bootstrap activated a test profile, restarted the "
                    ...
                    + "`combinedIndexSelectedClasses` entries, then compare `contextLoaderSelectedClasses`, "
                    + "`applicationArchives`, `applicationArchiveSelectedClasses`, `testClassBeanItems`, and "
                    + "`zoltAdditionalBeanItems` to separate profile-veto mismatches, missing index visibility, "
                    + "additional-bean classloader visibility, application-archive visibility, Arc input-item "
                    + "visibility, and the remaining Arc bean-registration handoff before @TestProfile can be enabled. "
