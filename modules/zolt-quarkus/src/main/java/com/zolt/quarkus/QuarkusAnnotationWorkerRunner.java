        ...
        if (!buildChainBuilderClassloaderSplit(result.output())) {
            ...
            if (profileTestClassBeanMissing(result.output())) {
                return "error: Quarkus annotation test bootstrap activated a test profile, restarted the "
                    ...
                    + "`applicationArchives`, and `applicationArchiveSelectedClasses` to separate profile-veto "
                    + "mismatches, missing index visibility, additional-bean classloader visibility, "
                    + "application-archive visibility, and the remaining Arc bean-registration handoff before "
