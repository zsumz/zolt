...
public final class ClasspathLaneAuditFormatter {
    public String formatText(ZoltLockfile lockfile) {
        ...
        output.append("scope               compile runtime test processor test-processor tool-openapi tool-protobuf tool-coverage package-default disposition\n");
        for (DependencyScope scope : scopes()) {
            output.append("%-19s %-7s %-7s %-4s %-9s %-14s %-12s %-13s %-13s %-15s %s%n".formatted(
                    scope.lockfileName(),
